$(document).ready(function () {
    ...
    function getLearnerPreAssessmentOutputData() {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                preAssessmentLearnerOutputData =
                    response["preAssessmentOutputData"];
                displayLearnerPreAssessmentOutputData(
                    preAssessmentLearnerOutputData,
                );
            },
    ...
    function displayLearnerPreAssessmentOutputData(
        preAssessmentLearnerOutputData,
    ) {
        ...
        for (let i = 0; i < preAssessmentLearnerOutputData.length; i++) {
            const learner_pre_assessment_output_id =
                preAssessmentLearnerOutputData[i][
                    "learner_pre_assessment_output_id"
                ];
            const syllabus_id =
                preAssessmentLearnerOutputData[i]["syllabus_id"];
            const course_id = preAssessmentLearnerOutputData[i]["course_id"];
            const question_id =
                preAssessmentLearnerOutputData[i]["question_id"];
            ...
            const correct_answer =
                preAssessmentLearnerOutputData[i]["correct_answer"];
            ...
            const answerText =
                correctAns.length > 0 ? `Answer: ${correctAns[0]}` : "";
            ...
            const check_answer_style =
                isCorrect == 1 ? "border-darthmouthgreen" : "border-red-600";
        ...
        $("#nextPage").on("click", function () {
            if (
                currentPage <
                Math.ceil(
                    preAssessmentLearnerOutputData.length / questionsPerPage,
                )
            ) {
                currentPage++;
                displayQuestionsByPage(currentPage);
