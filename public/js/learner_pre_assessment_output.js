$(document).ready(function () {
    ...
    var preAssessmentLearnerOutputData = [];
    var currentPage = 1;
    var questionsPerPage = 5;
    var baseUrl = window.location.href;


    getLearnerPreAssessmentOutputData();
    
    var csrfToken = $('meta[name="csrf-token"]').attr('content');
    getLearnerData()

    function getLearnerPreAssessmentOutputData() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                preAssessmentLearnerOutputData = response['preAssessmentOutputData'];
                displayLearnerPreAssessmentOutputData(preAssessmentLearnerOutputData);
            },
    ...
    function displayLearnerPreAssessmentOutputData(preAssessmentLearnerOutputData) {

        // console.log(preAssessmentLearnerOutputData)
        var questionDataDisp = ``;
        var questionIndicatorDisp = ``;

        $('#isAnsweredMeter').empty();

        for (let i = 0; i < preAssessmentLearnerOutputData.length; i++) {
            const learner_pre_assessment_output_id = preAssessmentLearnerOutputData[i]['learner_pre_assessment_output_id'];
            const syllabus_id = preAssessmentLearnerOutputData[i]['syllabus_id'];
            const course_id = preAssessmentLearnerOutputData[i]['course_id'];
            const question_id = preAssessmentLearnerOutputData[i]['question_id'];
            const category = preAssessmentLearnerOutputData[i]['category'];
            const question = preAssessmentLearnerOutputData[i]['question'];
            const choices = preAssessmentLearnerOutputData[i]['all_choices'];
            const correct_answer = preAssessmentLearnerOutputData[i]['correct_answer'];
            const answer = preAssessmentLearnerOutputData[i]['answer'];
            const isCorrect = preAssessmentLearnerOutputData[i]['isCorrect'];

    
            //const correctAns = JSON.parse(correct_answer);
            //const choicesArray = JSON.parse(choices);
            
            //const correctAns = correct_answer ? JSON.parse(correct_answer) : ["null"];
            const correctAns = JSON.parse(correct_answer) || [];
            const choicesArray = choices ? JSON.parse(choices) : ["null"];


            const questionCount = i + 1; // Start counting from 1
            const answerText = correctAns.length > 0 ? `Answer: ${correctAns[0]}` : '';
            ...
            if (category == 'MULTIPLECHOICE') {
                ...
                questionDataDisp += `
                        ...
                        <div class="mt-5">
                            <h1 class="text-xl font-semibold"> Answer: ${answerText}</h1>
                        </div>
                    </div>
                `;
        ...
        $('#nextPage').on('click', function () {
            if (currentPage < Math.ceil(preAssessmentLearnerOutputData.length / questionsPerPage)) {
                currentPage++;
                displayQuestionsByPage(currentPage);
            }
        });
