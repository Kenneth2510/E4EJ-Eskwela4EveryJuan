    ...
    <section class="w-full h-screen md:w-3/4 lg:w-10/12">
        <div class="relative w-full h-full px-2 py-4 pt-24 overflow-hidden overflow-y-scroll rounded-lg shadow-lg md:pt-4">
                ...
                <div class="px-2">
                    ...
                    <div id="lesson_title_area" class="mb-4">
                        <div class="flex items-center justify-between pb-3 my-4 mt-5 border-b-2 border-seagreen">
                            ...
                            <div id="edit_lesson_btns" class="hidden text-right">
                                <button id="save_lesson_btn" data-lesson-id="{{$lessonInfo->lesson_id}}" data-course-id="{{$lessonInfo->course_id}}"  data-syllabus-id="{{$lessonInfo->syllabus_id}}" class="px-5 py-3 m-1 text-white rounded-xl" style="background-color:{{$mainBackgroundCol}}" onmouseover="this.style.backgroundColor='{{$darkenedColor}}'" onmouseout="this.style.backgroundColor='{{$mainBackgroundCol}}'">
                                    Save
                                </button>
                        ...
                        <div id="estimatedCompletionTime" class="">
                            ...
                            <br>
                            <button id="saveEstTimeCompletion" class="px-5 py-3 mt-3 text-white bg-darthmouthgreen rounded-xl hover:ring-2 hover:ring-darthmouthgreen hover:text-darthmouthgreen hover:bg-white">Save Time Duration</button>
                        </div>
                    ...
                    <div class="mt-5">
                        ...
                        <div id="edit_lesson_picture_btns" style="position: relative; top: 75%;" class="flex justify-end hidden">
                            <button id="" data-lesson-id="{{$lessonInfo->lesson_id}}" data-course-id="{{$lessonInfo->course_id}}"  data-syllabus-id="{{$lessonInfo->syllabus_id}}" class="flex px-5 py-3 mr-3 text-white add_lesson_picture_btn rounded-xl" style="background-color:{{$mainBackgroundCol}}" onmouseover="this.style.backgroundColor='{{$darkenedColor}}'" onmouseout="this.style.backgroundColor='{{$mainBackgroundCol}}'">
                                Change Photo
                            </button>
                        </div>
                        @else
                    ...
                    <div class="flex justify-center w-full">
                        <button id="editLessonBtn" data-course-id="{{$lessonInfo->course_id}}" data-syllabus-id="{{$lessonInfo->syllabus_id}}"  style="background-color:{{$mainBackgroundCol}}" onmouseover="this.style.backgroundColor='{{$darkenedColor}}'"
                        ...
                        <div id="editBtns" class="flex justify-end hidden w-full">
                            ...
                            onmouseout="this.style.backgroundColor='{{$mainBackgroundCol}}'" class="w-1/2 p-3 mx-1 mt-4 text-white rounded-lg shadow-lg md:w-auto bg-seagreen hover:bg-green-800 hover:text-white" data-lesson-id="{{$lessonInfo->lesson_id}}" data-course-id="{{$lessonInfo->course_id}}" data-syllabus-id="{{$lessonInfo->syllabus_id}}" >
            ...
            <div class="hidden">
                ...
                <h3 style="font-size: 1.5rem; font-weight: semibold;">{{ $course->course_id }}</h3>
    ...
    <div class="z-50 w-11/12 mx-auto overflow-y-auto bg-white rounded shadow-lg modal-container md:max-w-md">
        <div class="px-6 py-4 text-left modal-content">
            ...
            <div class="mb-4">
                ...
                <form id="pictureUploadForm" data-lesson-id="{{$lessonInfo->lesson_id}}" data-course-id="{{$lessonInfo->course_id}}"  data-syllabus-id="{{$lessonInfo->syllabus_id}}" enctype="multipart/form-data" method="POST">
                    <!--<input type="file" name="picture" id="lesson_title_picture" accept=".jpeg, .png, .jpg, .gif" />-->
                    <input type="file" class="w-full max-w-xs file-input file-input-bordered file-input-primary" name="picture" id="lesson_title_picture" accept=".jpeg, .png, .jpg, .gif" />
...
<div id="lesson_content_pictureModal" class="z-[99] fixed inset-0  flex items-center justify-center hidden">
    <div class="z-50 w-11/12 mx-auto overflow-y-auto bg-white rounded shadow-lg modal-container md:max-w-md">
        <div class="px-6 py-4 text-left modal-content">
            ...
            <div class="mb-4">
                <!-- Your form for uploading pictures goes here -->
                <form id="lesson_content_pictureUploadForm" data-lesson-id="{{$lessonInfo->lesson_id}}" data-course-id="{{$lessonInfo->course_id}}"  data-syllabus-id="{{$lessonInfo->syllabus_id}}" enctype="multipart/form-data" method="POST">
                    <!--<input type="file" name="picture" id="lesson_title_picture" accept=".jpeg, .png, .jpg, .gif" />-->
                    <input type="file" class="w-full max-w-xs file-input file-input-bordered file-input-primary" name="picture" id="lesson_title_picture" accept=".jpeg, .png, .jpg, .gif" />

                    <div class="flex justify-between mt-4">
                        <button type="submit" class="btn btn-primary">Confirm</button>
                        <button id="cancelUpload_lesson_content_picture" class="btn btn-danger">Cancel</button>
                    </div>
                </form>
            </div>
        </div>
...
<div id="deleteLessonContentModal" class=" z-[99] fixed top-0 left-0 flex items-center hidden justify-center w-screen h-screen bg-black bg-opacity-50">
    {{-- <form id="deleteCourse" action="" data-course-id="{{ $course->course_id }}"> --}}
        {{-- @csrf --}}
        <div class="p-5 text-center bg-white rounded-lg">
            <p>Are you sure you want to delete this content?</p>
            <button type="button" id="confirmDelete" data-course-id="{{$course->course_id}}" class="btn btn-danger">Confirm</button>
...
<div id="deleteLessonContentPictureModal" class="z-[99] fixed top-0 left-0 flex items-center justify-center hidden w-screen h-screen bg-black bg-opacity-50">
    {{-- <form id="deleteCourse" action="" data-course-id="{{ $course->course_id }}"> --}}
        ...
        <div class="p-5 text-center bg-white rounded-lg">
            <p>Are you sure you want to delete this content?</p>
            <button type="button" id="confirmDelete_lessonContentPicture" data-course-id="{{$course->course_id}}" data-syllabus-id="{{$lessonInfo->syllabus_id}}"  class="px-4 py-2 m-2 text-white bg-red-600 rounded-md">Confirm</button>
...
<!--<div id="addLessonContentModal" class="fixed z-[99] top-0 left-0 flex items-center justify-center w-full h-full bg-gray-200 bg-opacity-75">-->
<!--    <div class="modal-content bg-white p-4 rounded-lg shadow-lg w-[750px]">-->
<!--        <div class="flex justify-end w-full">-->
...
<div id="addLessonContentModal" class="fixed z-[99] top-0 left-0 flex items-center justify-center hidden w-full h-full bg-gray-200 bg-opacity-75">
    <div class="w-full p-4 overflow-auto bg-white rounded-lg shadow-lg modal-content h-3/4 lg:w-1/2">
        ...
        <div class="flex justify-center w-full mt-5 space-x-2">
            <button id="confirmAddLessonContentBtn" data-lesson-id="{{$lessonInfo->lesson_id}}" class="btn btn-primary">Confirm</button>
            <button id="cancelAddLessonContentBtn" class="btn btn-danger">Cancel</button>
        </div>
    </div>
</div>
...
<div id="editLessonContentModal" class="fixed top-0 z-[99] left-0 flex items-center justify-center hidden w-full h-full bg-gray-200 bg-opacity-75">
    <div class="w-full p-4 overflow-auto bg-white rounded-lg shadow-lg modal-content h-3/4 lg:w-1/2">
        ...
        <div class="flex justify-center w-full mt-5 space-x-2">
            <button id="confirmEditLessonContentBtn" data-content-order="" data-lesson-id="{{$lessonInfo->lesson_id}}" class="btn btn-primary">Confirm</button>
            <button id="" class="btn btn-danger">Cancel</button>
        </div>
    </div>
</div>
...
<div id="addLessonContentUrlModal" class="fixed top-0 left-0 z-[99] flex items-center justify-center hidden w-full h-full bg-gray-200 bg-opacity-75">
    <div class="w-full p-4 overflow-auto bg-white rounded-lg shadow-lg modal-content lg:w-1/2">
        <div class="flex justify-end w-full">
        ...
        <div class="flex justify-center w-full mt-5 space-x-2">
            <button id="confirmAddLessonContentUrlBtn" data-content-order="" data-lesson-id="{{$lessonInfo->lesson_id}}" class="btn btn-primary">Confirm</button>
            <button id="cancelAddLessonContentUrlBtn" class="btn btn-danger">Cancel</button>
        </div>
    </div>
</div>
...
<div id="deleteLessonContentUrlModal" class="fixed top-0 z-[99] left-0 flex items-center justify-center hidden w-screen h-screen bg-black bg-opacity-50">
    {{-- <form id="deleteCourse" action="" data-course-id="{{ $course->course_id }}"> --}}
        ...
        <div class="p-5 space-x-2 text-center bg-white rounded-lg">
            <p>Are you sure you want to delete this content?</p>
            <button type="button" id="confirmDelete_lessonContentUrl" data-content-order="" data-lesson-content-id="" data-lesson-id="" data-course-id="{{$course->course_id}}" data-syllabus-id="{{$lessonInfo->syllabus_id}}"  class="btn btn-danger">Confirm</button>
            <button type="button" id="cancelDelete_lessonContentUrl" class="btn">Cancel</button>
        </div>
    {{-- </form> --}}
