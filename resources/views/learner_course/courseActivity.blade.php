...
<section class="w-full h-screen md:w-3/4 lg:w-10/12">
    <div class="h-full px-2 py-4 pt-24 overflow-hidden overflow-y-scroll rounded-lg shadow-lg md:pt-6">
        ...
        <div style="background-color:{{$mainBackgroundCol}};" class="z-50 p-2 text-white fill-white rounded-xl">
            ...
            <h1 class="w-1/2 py-4 text-2xl font-bold md:text-3xl lg:text-4xl"><span class="">{{ $syllabus->course_name
                    }}</span></h1>
            {{-- subheaders --}}
            <div class="flex flex-col justify-between fill-mainwhitebg">
                <h1 class="w-1/2 py-4 text-lg font-bold md:text-xl"><span class="">{{ $syllabus->activity_title
                        }}</span></h1>
            </div>
        </div>
        ...
        <div class="mx-2">
            ...
            <div class="flex flex-col justify-between py-4 border-b-2 lg:flex-row">
                <div class="flex flex-row items-center">
                    ...
                    <h1 class="mx-2 text-2xl font-semibold" id="activity_title"
                        data-course-id="{{$syllabus->course_id}}" data-syllabus-id="{{$syllabus->syllabus_id}}">
                        {{$syllabus->activity_title}}</h1>
                </div>
        ...
        <div class="px-10 mt-8" id="score_area">
            ...
            <div class="p-6 bg-gray-100 shadow-md rounded-xl">
                ...
                <h1 class="text-4xl font-bold text-green-600">{{$output->total_score}} <span
                        class="text-2xl font-bold text-black"> / {{$totalScores}}</span></h1>

                <div class="my-5">
                    <h1 class="text-xl font-semibold">Remarks:</h1>

                    <span
                        class="mx-2 text-2xl font-semibold {{ $output->mark == 'PASS' ? 'text-dartmouthgreen' : 'text-red-600' }}">
                        {{ $output->mark }}
                    </span>
                    </h1>
        ...
        <div class="flex mt-5 space-x-2">
            ...
            <!--commenting this to stop from redirecting-->
            <!--href="{{ url("/learner/course/content/$syllabus->course_id/$syllabus->learner_course_id/activity/$syllabus->syllabus_id/answer/$output->attempt") }}"-->
            <a class="w-1/2 cursor-not-allowed">
                <button class="w-full btn btn-disabled ">Answer Now</button>
            </a>
            @else {{-- if mark is FAIL --}}
            <a href="{{ url("/learner/course/manage/$syllabus->course_id/overview") }}" class="w-1/2">
                <button class="w-full btn">Return</button>
            </a>
            <a disabled href="" class="w-1/2 cursor-not-allowed ">
                <button class="w-full btn btn-disabled ">Reattempt Now</button>
            </a>
            @endif
            ...
            <a disabled href="" class="w-1/2 cursor-not-allowed">
                <button class="w-full btn btn-disabled">Already Submitted</button>
            </a>
            @else {{-- if mark is FAIL --}}
            ...
            <a disabled href="" class="w-1/2cursor-not-allowed">
                <button class="w-full btn btn-disabled">Already Submitted</button>
            </a>
            @endif
            ...
            @else
            <!-- if all null -->
            <a href="{{ url("/learner/course/manage/$syllabus->course_id/overview") }}" class="flex justify-center w-1/2">
                <button class="w-full btn">Return</button>
            </a>
            <a href="{{ url("/learner/course/content/$syllabus->course_id/$syllabus->learner_course_id/activity/$syllabus->syllabus_id/answer/1")
                }}" class="w-1/2">
                <button class="w-full btn btn-primary">Answer Now</button>
            </a>
        ...
        {{-- <div class="px-10 mt-[50px] flex justify-between">
            ...
            @if ($activity->status === "NOT YET STARTED")
            <a href="{{ url("/learner/course/content/$syllabus->course_id/$syllabus->learner_course_id/activity/$syllabus->syllabus_id/answer")
