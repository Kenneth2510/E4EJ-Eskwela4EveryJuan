...
<section class="w-full h-screen md:w-3/4 lg:w-10/12">
    <div class="h-full px-2 py-4 pt-24 overflow-hidden overflow-y-scroll rounded-lg shadow-lg md:pt-6">
        ...
        <div style="background-color:{{$mainBackgroundCol}};" class="p-2 text-white fill-white rounded-xl">
            <a href="{{ url("/learner/course/manage/$courseData->course_id/overview") }}" class="my-2 bg-gray-300
                rounded-full ">
                <svg xmlns="http://www.w3.org/2000/svg" height="30" viewBox="0 -960 960 960" width="24">
                    <path d="M560-240 320-480l240-240 56 56-184 184 184 184-56 56Z" />
                </svg>
            </a>
            <h1 class="w-1/2 py-4 text-2xl font-bold md:text-3xl lg:text-4xl"><span class="">{{ $courseData->course_name
                    }}</span></h1>
            {{-- subheaders --}}
        ...
        <div class="mx-2">
            ...
            <div class="flex flex-col justify-between py-4 border-b-2 lg:flex-row">
                <div class="flex flex-row items-center">
                    ...
                    <h1 class="mx-2 text-2xl font-semibold" id="gradesheetTitle"
                        data-course-id="{{$courseData->course_id}}">Gradesheet</h1>
                </div>
        ...
        <div class="mt-10">
            ...
            <h1 class="py-5 mx-16 text-4xl font-bold text-green-600">{{$preAssessmentData->score}} <span
                    class="text-2xl font-bold text-black"> / {{$totalScoreCount_pre_assessment}}</span></h1>
            <h1 class="py-5 mx-16 text-2xl font-bold text-green-600">Date Taken: <span
                    class="text-2xl font-bold text-black"> {{$preAssessmentData->finish_period}}</span></h1>
        </div>
                ...
                <table class="table w-full py-5 text-center table-fixed">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $activity->finish_period }}</td>
                            <td>{{ $activity->average_score }} / {{$activity->total_score}}</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
                ...
                <table class="table w-full py-5 text-center table-fixed">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $quiz->latest_finish_period }}</td>
                            <td>{{ $quiz->average_score }} / {{$quiz->total_score}}</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
        ...
        <div class="mt-10">
            ...
            <h1 class="py-5 mx-16 text-4xl font-bold text-green-600">{{$postAssessmentGrade->average_score}} <span
                    class="text-2xl font-bold text-black"> / {{$totalScoreCount_post_assessment}}</span></h1>
            <h1 class="py-5 mx-16 text-2xl font-bold text-green-600">Date Taken: <span
                    class="text-2xl font-bold text-black"> / {{$postAssessmentGrade->latest_finish_period}}</span></h1>
        </div>
        ...
        <div class="px-10 mt-3">
            ...
            <p class="py-5 mx-16 text-xl font-bold">[[ {{ $activityLearnerSumScore }} / {{ $activityTotalSum }} ] x 100
                ] x 35% = {{ $activityGrade }}%</p>


            <h1 class="text-xl font-bold">Quizzes</h1>
            <p class="py-5 mx-16 text-xl font-bold">[[ {{ $quizLearnerSumScore }} / {{ $quizTotalSum }} ] x 100 ] x 35%
                = {{ $quizGrade }}%</p>


            <h1 class="text-xl font-bold">Post Assessment</h1>
            <p class="py-5 mx-16 text-xl font-bold">[[ {{ $postAssessmentLearnerSumScore }} / {{
                $totalScoreCount_post_assessment }} ] x 100 ] x 30% = {{ $postAssessmentScoreGrade }}%</p>

            <h1 class="text-xl font-bold text-green-600">Overall Grade</h1>
            <p class="py-5 mx-16 text-xl font-bold text-green-600">{{ $activityGrade }} + {{ $quizGrade }} +
                {{$postAssessmentScoreGrade}} = {{ $totalGrade }}%</p>
