...
<section class="w-full h-screen md:w-3/4 lg:w-10/12">
    <div class="h-full px-2 py-4 pt-24 overflow-hidden overflow-y-scroll rounded-lg shadow-lg md:pt-6">
        ...
        <div class="mx-2">
            ...
            <div class="flex flex-col justify-between py-4 border-b-2 lg:flex-row">
                ...
                <div class="text-right">
                    ...
                    @if ($activityOutput)
                    <p class="px-4 text-4xl">{{$activityOutput->total_score ?? 'N/A'}} / {{$activity->total_score ??
                        'N/A'}}</p>
                    @else
                    ...
                    <table class="table w-full table-fixed rounded-xl">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <input type="text" class="flex text-center"
                                        value="{{ $activityScore[$index]->score }}" disabled>
                                </td>
                                @else
                                <td></td>
                                @endif
        ...
        <div class="w-full px-5">
            ...
            <textarea name="" style="white-space: pre-wrap"
                class="mt-5 px-5 py-5 h-[300px] w-full rounded-xl border-2 border-black"
                readonly>{{$activityOutput->answer}}</textarea>
            @else
            <textarea id="activity_answer" name="activity_answer" style="white-space: pre-wrap"
                class="mt-5 px-5 py-5 h-[300px] w-full rounded-xl border-2 border-black"></textarea>
            @endif
        </div>
        ...
        <div class="px-10 mt-[50px] flex justify-center items-center space-x-2">
            <a href="{{ url("/learner/course/content/$activity->course_id/$activity->learner_course_id/activity/$activity->syllabus_id")
                }}" class="w-1/2">
...
<div id="confirmationModal"
    class="fixed z-[99] top-0 left-0 flex items-center justify-center hidden w-screen h-screen bg-black bg-opacity-50">
    <div class="p-5 text-center bg-white rounded-lg">
        ...
        <div class="flex justify-end">
            ...
            <button id="confirmButton" data-learner-course-id="{{$activity->learner_course_id}}"
                data-course-id="{{$activity->course_id}}" data-syllabus-id="{{$activity->syllabus_id}}"
