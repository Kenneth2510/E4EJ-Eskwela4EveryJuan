...
<section class="flex flex-row w-full h-screen text-sm bg-mainwhitebg md:text-base lg:h-screen">
    ...
    <section class="w-full pt-[125px] mx-4  overscroll-auto md:overflow-auto">
        ...
        <div class="relative z-0 pb-4 bg-black border border-gray-400 rounded-lg shadow-lg text-mainwhitebg">
            <img class="absolute top-0 left-0 object-cover w-full h-full pointer-events-none -z-10 opacity-30"
                src="{{asset('images/marketing-img.png')}}" alt="computer with microphone">
            <div class="z-50 p-2">
            ...
            <div class="flex items-center w-full">
                ...
                <x-forms.primary-button color="red" name="Unenroll" type="button" class="bg-red-600 hover:bg-red-700"
                    id="unenrollBtn" />
                {{-- <button type="button" id="unenrollBtn"
                    class="w-32 h-10 m-2 bg-red-600 rounded-full hover:bg-red-800">
                    Unenroll
                </button> --}}
                @else
                <x-forms.primary-button color="seagreen" name="Enroll Now" type="button" class="hover:bg-green-800"
                    id="enrollBtn" />
                {{-- <button type="button" id="enrollBtn"
                    class="w-32 h-10 m-2 rounded-full bg-seagreen hover:bg-green-800">
                    <h1>Enroll Now</h1>
                </button> --}}
                @endif
            </div>
        ...
        <div id="unenrollCourseModal"
            class="fixed top-0 left-0 z-30 flex items-center justify-center hidden w-screen h-screen bg-black bg-opacity-50">
            <form id="unenrollCourse" data-learner_course-id="{{ $isEnrolled->learner_course_id }}">
                ...
                <div class="p-5 text-center bg-white rounded-lg">
                    ...
                    <x-forms.primary-button color="red" name="Unenroll" class="bg-red-600 hover:bg-red-700"
                        id="confirmUnenroll" />
                    <x-forms.primary-button color="gray" name="Cancel" type="button" id="cancelUnenroll" />
                    {{-- <button type="submit" id="confirmUnenroll"
                        class="px-4 py-2 m-2 text-white bg-red-600 rounded-md">
                        Unenroll
                    </button>
                    <button type="button" id="cancelUnenroll"
                        class="px-4 py-2 m-2 text-gray-700 bg-gray-400 rounded-md">
                        Cancel
                    </button> --}}
                </div>
            </form>
        ...
        <div id="enrollCourseModal"
            ...
            <form id="enrollCourse" action="" data-course-id="{{$course->course_id}}">
                ...
                <div class="p-5 text-center bg-white rounded-lg">
                    ...
                    <x-forms.primary-button color="green" name="Confirm" class="bg-green-600 hover:bg-green-700"
                        id="confirmEnroll" />
                    <x-forms.primary-button color="gray" name="Cancel" type="button" id="cancelEnroll" />
                    {{-- <button type="submit" id="confirmEnroll"
                        class="px-4 py-2 m-2 text-white bg-green-600 rounded-md">Confirm</button>
        ...
        <div class="relative w-full mt-5">
            ...
            <div class="flex justify-between text-mainwhitebg fill-mainwhitebg">
                <a class="relative w-1/2 h-16 p-2 mr-2 text-center rounded-lg bg-darthmouthgreen">
                    <h1>Manage Course</h1>
                    <svg class="absolute bottom-0 right-0 hidden mx-2 " xmlns="http://www.w3.org/2000/svg" height="24"
                ...
                <div id="content_area" class="w-5/6 m-5 overflow-y-auto ">
                            ...
                            <div id="info" class="mt-5 overflow-y-auto">
                                <div class="flex">
                                    <div class="w-2/5">
                                        <div class="flex my-2 justify-normal">
                                            ...
                                            <input type="text" value="{{ $course->course_id }}" class="w-4/6 text-lg"
                                                disabled>
                                        </div>
                                        <div class="flex my-2 justify-normal ">
                                            <label for="course_name" class="w-2/6 text-lg">Course Name:</label>
                                            <input type="text" id="course_name" name="course_name"
                                                value="{{ $course->course_name }}" class="w-4/6 text-lg" disabled>
                                        </div>
                                    </div>
                                    ...
                                    <div class="w-2/5 mx-5">
                                        ...
                                        <div class="flex py-1 my-1 justify-normal">
                                            ...
                                            <select name="course_difficulty" id="course_difficulty" class="w-2/5"
                                                disabled>
                                                <option value="" {{ $course->course_difficulty == '' ? 'selected' : ''
                                                    }}>--select an option--</option>
                                                <option value="Beginner" {{ $course->course_difficulty == 'Beginner' ?
                                                    'selected': '' }}>Beginner</option>
                                                <option value="Intermediate" {{ $course->course_difficulty ==
                                                    'Intermediate' ? 'selected': '' }}>Intermediate</option>
                                                <option value="Advanced" {{ $course->course_difficulty == 'Advanced' ?
                                    ...
                                    <div class="mt-5">
                                        <h1>Created {{ $course->created_at }} by {{ $course->instructor_fname }} {{
                                            $course->instructor_lname }}</h1>
                                ...
                                <div class="mt-1">
                                    ...
                                    <textarea name="course_description" id="course_description"
                                        class="w-full h-24 max-w-full max-h-24"
                                        disabled>{{ $course->course_description }}</textarea>
                                </div>
                    ...
                    <div id="learner_enrolled_learners" class="hidden">
                        ...
                        <form id="enrolleeForm" data-course-id="{{$course->course_id}}"
                            action="/learner/course/manage/{{$course->course_id}}" method="GET">
                            <div class="flex items-center">
                                <div class="flex items-center mx-10">
                                    <div class="mx-2">
                                        <label for="filterDate" class="">Filter by Date</label><br>
                                        <input type="date" name="filterDate"
                                            class="w-40 px-2 py-2 text-base border-2 border-black rounded-xl"
                                            value="{{ request('filterDate') }}">
                                    </div>
                                    <div class="mx-2">
                                        <label for="filterStatus" class="">Filter by Status</label><br>
                                        <select name="filterStatus" id="filterStatus"
                                            class="w-32 px-2 py-2 text-base border-2 border-black rounded-xl">
                                            <option value="" {{ request('filterDate')=='' ? 'selected' : '' }}>Select
                                                Status</option>
                                            <option value="Pending" {{ request('filterStatus')=='Pending' ? 'selected'
                                                : '' }}>Pending</option>
                                            <option value="Approved" {{ request('filterStatus')=='Approved' ? 'selected'
                                                : '' }}>Approved</option>
                                            <option value="Rejected" {{ request('filterStatus')=='Rejected' ? 'selected'
                                                : '' }}>Rejected</option>
                                        </select>
                                    </div>
                                    <button
                                        class="h-12 px-5 py-1 mx-3 text-lg font-medium bg-green-600 rounded-xl hover:bg-green-900 hover:text-white"
                                        type="submit">Filter</button>
                                </div>
                                <div class="">
                                    <select name="searchBy" id=""
                                        class="w-40 px-2 py-2 text-lg border-2 border-black rounded-xl">
                                        <option value="" {{request('searchBy')=='' ? 'selected' : '' }}class="">Search
                                            By</option>
                                        <option value="learner_course_id" {{request('searchBy')=='learner_course_id'
                                            ? 'selected' : '' }}>Enrollee ID</option>
                                        <option value="learner_id" {{request('searchBy')=='learner_id' ? 'selected' : ''
                                            }}>Learner ID</option>
                                        <option value="name" {{request('searchBy')=='name' ? 'selected' : '' }}>Name
                                        </option>
                                        <option value="learner_email" {{request('searchBy')=='learner_email'
                                            ? 'selected' : '' }}>Email</option>
                                        <option value="learner_contactno" {{request('searchBy')=='learner_contactno'
                                            ? 'selected' : '' }}>Contact No.</option>
                                        {{-- <option value="created_at">Date Registered</option> --}}
                                        {{-- <option value="status">Status</option> --}}
                                    </select>
                                    <input type="text" name="searchVal"
                                        class="px-2 py-2 ml-3 text-lg border-2 border-black w-80 rounded-xl"
                                        value="{{ request('searchVal') }}" placeholder="Type to search">
                                    <button
                                        class="px-3 py-2 mx-3 text-lg font-medium bg-green-600 rounded-xl hover:bg-green-900 hover:text-white"
                    ...
                    <div id="enrollment_summary" class="hidden overflow-y-auto">
                        ...
                        {{-- <div class="flex justify-end">
                            <button type="button" id="unenrollBtn2"
                                class="w-32 h-10 m-2 bg-red-600 rounded-full hover:bg-red-800">
                                Unenroll
                            </button>
                        </div>
                        ...
                        <div id="unenrollCourseModal2"
                            class="fixed top-0 left-0 z-30 flex items-center justify-center hidden w-screen h-screen bg-black bg-opacity-50">
                            <form id="unenrollCourse2" data-learner_course-id="{{ $isEnrolled->learner_course_id }}">
                                @csrf
                                <div class="p-5 text-center bg-white rounded-lg">
                                    <p>Are you sure you want to unenroll the course?</p>
                                    <button type="submit" id="confirmUnenroll2"
                                        class="px-4 py-2 m-2 text-white bg-red-600 rounded-md">
                                        Unenroll
                                    </button>
                                    <button type="button" id="cancelUnenroll2"
                                        class="px-4 py-2 m-2 text-gray-700 bg-gray-400 rounded-md">
                                        Cancel
                                    </button>
                                </div>
                        ...
                        {{-- <div class="flex justify-end">
                            <button id="showDeleteModal"
                                class="px-5 py-5 text-xl bg-red-600 rounded-xl hover:bg-red-700">Delete Course</button>
                        </div>
