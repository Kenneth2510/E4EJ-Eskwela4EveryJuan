            ...
            <div class="mx-2">
                ...
                <div class="py-4 " id="defaultView">
                    ...
                    <textarea name="activity_instructions"
                        class="w-full max-w-full min-w-full activity_instructions h-[200px]"
                        disabled>{{$activity->activity_instructions}}</textarea>


                    <div class="flex flex-row items-center border-b-2 border-b-green-900">
                        <h3 class="my-2 text-2xl font-medium">{{$learnerActivityOutput->learner_fname}}
                            {{$learnerActivityOutput->learner_lname}}'s output</h3>
                    ...
                    <textarea name="activity_instructions"
                        class="border-2 border-black mt-2 rounded-xl p-3 w-full max-w-full min-w-full activity_instructions h-[200px]"
                        disabled>{{$learnerActivityOutput->answer}}</textarea>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="flex justify-end">
                                    <input type="text" class="flex text-center" value="{{ $criteria->criteria_score }}"
                                        disabled>
                                </td>
                                <td>
                                    <input type="number"
                                        ...
                                        max="{{ $criteria->criteria_score }}" min="0" value="{{$criteria->score}}"
                                        disabled>
                                </td>
                            </tr>
                            @empty
                    ...
                    <div id="remarks_area" class="">
                        ...
                        <textarea name="remarks" id="remarks"
                            class="border-2 border-gray-200 rounded-xl px-3 py-3 w-full max-w-full min-w-full activity_instructions h-[200px]"
                            disabled>{{$learnerActivityOutput->remarks}}</textarea>
                    </div>

                    <div class="p-6 bg-gray-100 shadow-md rounded-xl">
                        <h1 class="mb-4 text-3xl font-bold">Score:</h1>
                        <div class="flex items-center">
                            <input type="number" id="overallScore_input"
                                class="px-5 py-5 text-4xl font-semibold text-darthmouthgreen"
                                value="{{$learnerActivityOutput->total_score}}" max="{{$activity->total_score}}" min="0"
                                disabled>
                        ...
                        <div class="my-5">
                            ...
                            <span
                                class="mx-2 text-4xl font-semibold {{ $learnerActivityOutput->mark == 'PASS' ? 'text-darthmouthgreen' : 'text-red-600' }}">
                ...
                <div class="flex justify-between w-full px-3 mx-3">
                    <a id="returnBtn"
                        href="/admin/courseManage/content/{{$activity->course_id}}/{{$activity->syllabus_id}}}/activity"
                        class="w-1/2 px-5 py-5 mx-2 text-xl text-center text-white bg-green-700 rounded-xl hover:bg-green-900">Return</a>
                    <button id="cancelScoreBtn"
                        class="hidden w-1/2 px-5 py-5 mx-2 text-xl text-center text-white bg-green-600 rounded-xl hover:bg-green-900">Cancel</button>
                    <button id="addScoreBtn"
                        class="w-1/2 px-5 py-5 mx-2 text-xl text-center text-white bg-green-600 rounded-xl hover:bg-green-900">Add
                        Score</button>
                    <button id="submitScoreBtn"
                        class="hidden w-1/2 px-5 py-5 mx-2 text-xl text-center text-white bg-green-600 rounded-xl hover:bg-green-900">Submit
                        Score</button>
                </div>
            </div>
            ...
            <div class="flex flex-col items-start p-6">
                ...
                <div class="flex justify-end w-full mt-6">
                    <button id="confirmSubmit"
                        ...
                        data-attempt="{{$learnerActivityOutput->attempt}}">Yes</button>
                    <button id="cancelSubmit"
                        class="px-4 py-2 ml-4 text-gray-700 bg-gray-300 rounded-md hover:bg-gray-400 focus:outline-none">Cancel</button>
                </div>
            </div>
        </div>
    </div>
