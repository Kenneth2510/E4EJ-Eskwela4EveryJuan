    ...
    public function pre_assessment(Course $course, LearnerCourse $learner_course) {
        if (session()->has('learner')) {
            ...
            try {
                ...
                $preAssessmentDurationInMillis = $preAssessmentData->max_duration;
                $preAssessmentDurationInSeconds = $preAssessmentDurationInMillis / 1000;
                $formattedDuration = gmdate('H:i:s', $preAssessmentDurationInSeconds);
    ...
    public function view_quiz(Course $course, LearnerCourse $learner_course, Syllabus $syllabus) {
        ...
        if (session()->has('learner')) {
            ...
            try {
                ...
                $quizDurationInMillis = $learnerSyllabusProgressData->duration;
                $quizDurationInSeconds = $quizDurationInMillis / 1000;
                $formattedDuration = gmdate('H:i:s', $quizDurationInSeconds);
    ...
    public function post_assessment(Course $course, LearnerCourse $learner_course) {
        if (session()->has('learner')) {
            ...
            try {
                ...
                $postAssessmentDurationInMillis = $postAssessmentData_recent->max_duration;
                $postAssessmentDurationInSeconds = $postAssessmentDurationInMillis / 1000;
                $formattedDuration = gmdate('H:i:s', $postAssessmentDurationInSeconds);
    ...
    public function view_output_post_assessment_json(Course $course, LearnerCourse $learner_course, $attempt) {
        if (session()->has('learner')) {
            ...
            try {
                ...
                ->where('learner_post_assessment_output.course_id', $courseData->course_id)
                ->where('learner_post_assessment_output.learner_course_id', $courseData->learner_course_id)
                ->where('learner_post_assessment_output.attempt', $attempt)
                ->groupBy(
                    'learner_post_assessment_output.learner_post_assessment_output_id',
                    'learner_post_assessment_output.learner_course_id',
                    'learner_post_assessment_output.course_id',
...
$learnerActivityScoresData = DB::table('learner_activity_output')
    ...
    ->join('learner_activity_progress', function($join) use ($learner_course) {
        $join->on('learner_activity_progress.activity_id', '=', 'learner_activity_output.activity_id')
             ->where('learner_activity_progress.learner_course_id', '=', $learner_course->learner_course_id);
    })
    ->leftJoin('activity_content', 'activity_content.activity_id', 'learner_activity_output.activity_id')
    ->where('learner_activity_output.course_id', $course->course_id)
    ->where('learner_activity_output.learner_course_id', $learner_course->learner_course_id)
    ->groupBy('learner_activity_output.activity_id', 'learner_activity_output.activity_content_id', 'activities.activity_title', 'learner_activity_progress.finish_period', 'activity_content.total_score')
    ->get();
...
$learnerQuizScoresData = DB::table('learner_quiz_progress')
    ->select(
        ...
        'quizzes.quiz_title',
        DB::raw('COALESCE(ROUND(AVG(learner_quiz_progress.score), 2), 0) as average_score'),
        DB::raw('MAX(learner_quiz_progress.finish_period) as latest_finish_period'),
        DB::raw('(SELECT COUNT(*) FROM learner_quiz_output WHERE learner_quiz_output.quiz_id = learner_quiz_progress.quiz_id AND learner_quiz_output.learner_course_id = '.$learner_course->learner_course_id.') as total_score')
    )
    ->leftJoin('quizzes', 'quizzes.quiz_id', '=', 'learner_quiz_progress.quiz_id')
    ->where('learner_quiz_progress.course_id', $course->course_id)
    ->where('learner_quiz_progress.learner_course_id', $learner_course->learner_course_id)
...
$learnerPostAssessmentGrade = DB::table('learner_post_assessment_progress')
    ->select (
        DB::raw('COALESCE(ROUND(AVG(IFNULL(score, 0)), 2), 0) as average_score'),
        DB::raw('MAX(finish_period) as latest_finish_period')
    )
    ->where('course_id', $course->course_id)
    ->where('learner_course_id', $learner_course->learner_course_id)
    ->first();
    ...
    public function generate_certificate(Course $course, LearnerCourse $learner_course) {
        if (session()->has('learner')) {
            ...
            try {
                ...
                and skill, demonstrating a commendable commitment to learning and personal\n
                development. She has effectively fulfilled the requirements for this program.\n
                Awarded on $formattedDate.
                \n \n
                Course Name: $learnerCourseProgressData->course_name \n
                Remarks: $learnerCourseProgressData->remarks";
